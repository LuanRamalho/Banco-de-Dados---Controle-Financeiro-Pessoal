...
def view_records():
    ...
    expenses_label = tk.Label(expenses_frame, text="Despesas", bg='lightblue')
    ...
    for i, expense in enumerate(data["expenses"]):
        ...
        record_label = tk.Label(record_frame, text=f"{i+1}. {expense['description']}: R$ {expense['amount']:.2f}", bg='lightblue')
        record_label.pack(side='left', padx=5)
        edit_button = tk.Button(record_frame, text="Editar", command=lambda i=i: edit_expense(i), bg='lightgray')
        edit_button.pack(side='right')
        delete_button = tk.Button(record_frame, text="Excluir", command=lambda i=i: delete_expense(i), bg='lightgray')
        delete_button.pack(side='right')

    incomes_frame = tk.Frame(records_window, bg='lightgreen', padx=10, pady=10)
    incomes_frame.pack(fill='x')
    incomes_label = tk.Label(incomes_frame, text="Receitas", bg='lightgreen')
    incomes_label.pack(fill='x')
    for i, income in enumerate(data["incomes"]):
        record_frame = tk.Frame(incomes_frame, bg='lightgreen')
        record_frame.pack(fill='x', padx=10, pady=5)
        record_label = tk.Label(record_frame, text=f"{i+1}. {income['description']}: R$ {income['amount']:.2f}", bg='lightgreen')
        record_label.pack(side='left', padx=5)
        edit_button = tk.Button(record_frame, text="Editar", command=lambda i=i: edit_income(i), bg='lightgray')
        edit_button.pack(side='right')
        delete_button = tk.Button(record_frame, text="Excluir", command=lambda i=i: delete_income(i), bg='lightgray')
        delete_button.pack(side='right')
...
expense_label = tk.Label(expense_frame, text="Adicionar Despesa", bg='lightblue')
expense_label.pack()
...
expense_button = tk.Button(expense_frame, text="Adicionar", command=add_expense, bg='lightgray')
expense_button.pack(pady=5)

# Seção de Receitas
income_frame = tk.Frame(root, padx=10, pady=10, bg='lightgreen')
income_frame.pack(fill='x')
income_label = tk.Label(income_frame, text="Adicionar Receita", bg='lightgreen')
income_label.pack()
...
income_button = tk.Button(income_frame, text="Adicionar", command=add_income, bg='lightgray')
income_button.pack(pady=5)

# Botões de Gráficos e Relatórios
buttons_frame = tk.Frame(root, padx=10, pady=10, bg='white')
buttons_frame.pack(fill='x')
graph_button = tk.Button(buttons_frame, text="Gerar Gráficos", command=generate_graphs, bg='lightgray')
graph_button.pack(side='left', padx=5)
report_button = tk.Button(buttons_frame, text="Gerar Relatório", command=generate_report, bg='lightgray')
report_button.pack(side='left', padx=5)
view_records_button = tk.Button(buttons_frame, text="Ver Registros", command=view_records, bg='lightgray')
